...
for (const person of PEOPLE) {
  ...
  const faceSize = radius * 1.66;
  face.scale.set(faceSize, faceSize, 1);
  face.userData.size = faceSize;
  face.renderOrder = 3;
  world.add(face);
...
const links = PEOPLE.filter((p) => p.parent).map((person) => {
  ...
  const line = new THREE.Line(
    geo,
    new THREE.LineBasicMaterial({ color: 0x8fb4cc, transparent: true, opacity: 0.72 }),
  );
  world.add(line);
  return { person, line };
});

function surfaceAnchor(from, to, radius) {
  const dir = to.clone().sub(from);
  const len = Math.max(dir.length(), 0.001);
  dir.multiplyScalar(1 / len);
  return from.clone().add(dir.multiplyScalar(radius + 0.06));
}

function writeLinks() {
  for (const link of links) {
    const a = balls.get(link.person.parent);
    const b = balls.get(link.person.id);
    const start = surfaceAnchor(a.position, b.position, a.userData.radius);
    const end = surfaceAnchor(b.position, a.position, b.userData.radius);
...
function showCard(person) {
  ...
  for (const mesh of nodes) {
    ...
    mesh.material.emissiveIntensity = on ? 0.22 : 0.04;
...
function clearCard() {
  ...
  for (const mesh of nodes) mesh.material.emissiveIntensity = 0.08;
}
...
function tick() {
  ...
  for (const mesh of nodes) {
    const { base, phase, radius, face } = mesh.userData;
    mesh.position.copy(base);
    if (!reducedMotion) {
      mesh.position.y = base.y + Math.sin(t * 0.85 + phase) * 0.16;
      mesh.rotation.y = t * 0.18 + phase;
    }
    const toCam = camera.position.clone().sub(mesh.position);
    if (toCam.lengthSq() > 0.0001) toCam.normalize();
    face.position.copy(mesh.position).add(toCam.multiplyScalar(radius * 1.02));
    face.scale.set(face.userData.size, face.userData.size, 1);
  }
  writeLinks();
  controls.update();
  projectLabels();
